...
export function Globe({
  ...
  return (
    <div className={clsx('absolute inset-0 aspect-square', className)}>
      <canvas className={clsx('h-full w-full', isGrabbing ? 'cursor-grabbing' : 'cursor-grab')} ref={canvasRef} onPointerDown={(event) => onPointerDown(event)} onPointerUp={(event) => onPointerUp(event)} onPointerOut={(event) => onPointerOut(event)} onMouseMove={(event) => onMouseMove(event)} onTouchMove={(event) => onTouchMove(event)} />
    </div>
  )
}
